...
impl App {
        ...
        where W: Write
    {
        ...
        for i in 0..(self.high + 1) {
            for j in 0..(self.width + 1){
                ...
                {
                    print!("*"); // 输出挡板
                } else if (i == self.block_y) && (j == self.block_x) {
                    print!("B"); // 输出砖块
                } else if i == self.high {
                    print!("-");
    ...
    pub fn update_without_input(&mut self) -> Result<String, String> {
        ...
        if (self.ball_x == self.block_x) && (self.ball_y == self.block_y) {
            self.score += 1;    // 分数加1
            self.block_x = thread_rng().gen_range(0, self.width); // 产生新的砖块
        }
