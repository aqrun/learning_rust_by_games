...
impl App {
    ...
    pub fn update_without_input(&mut self) -> Result<String, String> {
        ...
        if self.bar1_x <= 0 {
            self.bar1_x = self.width;
            let temp = thread_rng().gen_range(0, ((self.high as f32) * 0.8) as i32);
            self.bar1_y_bottom = temp - self.high / 10;
            self.bar1_y_top = temp + self.high / 10;
        }
